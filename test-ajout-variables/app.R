            ...
            input$newVars.ajout,
            {
                ...
                insertUI(
                    ...
                    fluidRow(
                        ...
                        column(width = 3, textInput(paste0("newVars", e, ".cut"), "Découpage", 5, placeholder = "indiquer ici les bornes des intervalles"))
                        #,
                        #column(width = 3, plotOutput(paste0("newVars", e, ".bar")))
                    )
                )
                
                # input[[paste0("newVars", e, ".bar")]] <- renderPlot({
                #     v = donnees()[,input[[paste0("newVars", e, ".var")]],]
                #     c = as.numeric(strsplit(input[[paste0("newVars", e, ".cut")]], ",")[[1]])
                #     r = cut(v, c)
                #     barplot(table(r))
                # })
            }
